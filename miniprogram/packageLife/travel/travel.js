...
Page({
  ...
  loadData(isRefresh = false) {
    ...
    dbUtil.getList({ collection: 'travel', page, pageSize })
      .then((res) => {
        if (page === 0) {
  ...
  publish() {
    ...
    if (!name) {
      wx.showToast({ icon: "none", title: '景点名称不能为空' })
      return
    }
    ...
    db.collection('travel').add({
      ...
      wx.hideLoading()
      wx.showToast({ title: '美景分享出去哒！' })
      this.setData({ isSend: false, name: null, didian: null, price: null, info: null })
      this.loadData(true)
    }).catch(err => {
      wx.hideLoading()
  ...
  deleteItem(e) {
    ...
    dbUtil.removeItem({ collection: 'travel', id: item._id })
      .then(() => {
  ...
  goDetail(e) {
    ...
    wx.navigateTo({
      url: '../../pages/detail/detail?name=travel&id=' + item._id
    })
  },
